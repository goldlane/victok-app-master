...
function EditChartAdmin() {
  const { id, id2, user_idx } = useParams();
  ...
  const getDrillerList = async (idx) => {
    try {
      console.log(idx);
      const res = await API.get('user/driller', { params: { driller: idx, user_idx: user_idx } });
  ...
  const onEditChart = async () => {
    ...
    console.log(form);
    try {
      const res = await API.put('customer/drilling-chart', form);
      console.log(res.data);
      Modal.success({
        title: '지공차트 저장 완료',
  ...
  const onEditChartName = async () => {
    if (!chartName)
      return Modal.error({
        title: '차트 제목 변경 실패',
        content: '차트 제목을 입력해 주세요.',
        okText: '확인',
      });
    const form = {
      chart_idx: id2,
    ...
    try {
      const res = await API.put('customer/drilling-chart-name', form);
      Modal.success({
        title: '차트 제목 수정 완료',
  ...
  useEffect(() => {
    Promise.all([getChartDetails()]);
  }, []);

  useEffect(() => {
    if (infoData) {
      getDrillerList(infoData.driller_idx);
    }
  }, [infoData]);
  return (
        ...
        <Box>
          <TitleWrap>
              ...
              {isEdit ? (
                <>
                  <Input
                    ...
                    style={{ width: '250px', height: '32px', marginRight: '10px', border: 0, borderBottom: '1px solid #ccc', fontSize: '20px' }}
                    maxLength={11}
                    placeholder="제목을 입력해 주세요."
                  />
                  <Button onClick={onEditChartName} style={{ fontSize: '14px' }}>
                    저장
                  </Button>
                </>
              ) : (
                <>
                  ...
                  <EditButton onClick={() => setIsEdit((prev) => !prev)}>
                    <EditOutlined style={{ fontSize: '18px', color: '#b1b1b1' }} />
                  </EditButton>
                </>
              )}
              <SubTitle style={{ marginLeft: '15px' }}>
            ...
            <Div>
              {buttonVisible && (
                <>
                  <Button onClick={onRegisterChart} type={'primary'} style={{ marginRight: '5px' }}>
                    다른 이름으로 저장
                  </Button>
                  <Button onClick={onEditChart} type={'primary'} style={{ marginRight: '5px' }}>
                    저장
                  </Button>
                </>
              )}
            ...
            <InfoWrap>
              ...
              <RowWrap>
                <Input
                  ...
                  onChange={(e) => {
                    setInfoData((prev) => ({ ...prev, ballName: e.target.value }));
                    setButtonVisible(true);
                  }}
                />
                <Input
                  ...
                  onChange={(e) => {
                    setInfoData((prev) => ({ ...prev, weight: e.target.value }));
                    setButtonVisible(true);
                  }}
                  type="number"
                />
              ...
              <RowWrap style={{ margin: '15px 0' }}>
                <Div style={{ marginRight: '35px' }}>
                  ...
                  <Div>
                    <Checkbox
                      checked={infoData.hand === 'right'}
                      onClick={() => {
                        setInfoData((prev) => ({ ...prev, hand: 'right' }));
                        setButtonVisible(true);
                      }}
                    >
                      오른손
                    ...
                    <Checkbox
                      checked={infoData.hand === 'left'}
                      onClick={() => {
                        setInfoData((prev) => ({ ...prev, hand: 'left' }));
                        setButtonVisible(true);
                      }}
                    >
                      왼손
                ...
                <Div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setInfoData((prev) => ({ ...prev, layout: e.target.value }));
                      setButtonVisible(true);
                    }}
                  />
                  <Div>
                    <Checkbox
                      checked={infoData.pin === 'up'}
                      onClick={() => {
                        setInfoData((prev) => ({ ...prev, pin: 'up' }));
                        setButtonVisible(true);
                      }}
                    >
                      핀업
                    ...
                    <Checkbox
                      checked={infoData.pin === 'down'}
                      onClick={() => {
                        setInfoData((prev) => ({ ...prev, pin: 'down' }));
                        setButtonVisible(true);
                      }}
                    >
                      핀다운
              ...
              <TextArea
                ...
                value={infoData.memo}
                onChange={(e) => {
                  setInfoData((prev) => ({ ...prev, memo: e.target.value }));
                  setButtonVisible(true);
                }}
              />
            </InfoWrap>
