import { Button, Form, Input, Modal, Checkbox, Divider, Radio } from 'antd';
...
function LockerFormEditModal({ visible, setVisible, getData, data, user_idx, grade }) {
  ...
  const onConfirm = async (values) => {
    ...
    try {
      const formdata = {
        locker_type_idx: visible.idx,
        locker_type: lockerInfo.name,
        locker_amount: lockerInfo.max,
        start_number: lockerInfo.startNum,
        talk_dday: talk_dday,
        charge: chargeList,
        user_idx: user_idx ? user_idx : undefined,
      };
      const res = await API.put(user_idx ? '/admin/locker-type' : '/locker/locker-type', formdata);
      Modal.success({ title: '라카 구분 등록 알림', content: '라카 수정이 완료되었습니다.', okText: '확인' });
      getData();
      onCancel();
    } catch (error) {
      if (axios.isAxiosError(error)) {
        console.log(error.response);
        Modal.error({ title: '라카 구분 등록 알림', content: `${error.response.data.message}`, okText: '확인' });
      }
    }
  };
  ...
  const onAddCharge = () => {
    ...
    console.log(duplication, periodType);
    if (!duplication) {
      const item = {
  ...
  const onCheck = async () => {
    ...
    const check = data.find((item) => item.locker_type === name && item.locker_type !== prevName);
    if (check) {
  ...
  useEffect(() => {
    if (visible) {
      form1.setFieldsValue({ name: visible.locker_type, max: visible.locker_amount, startNum: visible.start_number });
      setChargeList(visible.charge);
      setPrevName(visible.locker_type);
      form2.setFieldsValue({ talk_dday: visible.dday.split(',').map((item) => Number(item)) });
    }
  }, [visible]);

  return (
    <Modal visible={visible} title={`라카 구분 수정(${page}/2)`} onCancel={onCancel} footer={[]} maskClosable={false}>
...
export default LockerFormEditModal;
